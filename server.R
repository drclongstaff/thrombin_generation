...
shinyServer(function(input, output) { # Set up the Shiny Server
  ...
  CalibThrombin <- reactive({
    CalibThrombin <- CalibDat()[-1] %>%
      ...
      add_column(CalibDat()[, 1], .before = 1) %>%
      as.data.frame()
  ...
  readData <- reactive({
    ...
    plateFd <- plateFC[-1] %>% # select the raw fluorescent curves to make derivatives (d)
      map_df(~ fun_diff(.x, plateFC)) %>%
      add_column(plateFC[, 1][-1], .before = 1) %>%
      as.data.frame()
    # The first derivative curves are converted to thrombin (T)
    plateFdT <- plateFd[-1] %>%
      map_df(~ fun_FtoT(.x, input$CalibT, input$calSlope)) %>%
      add_column("time_d" = plateFd[[1]], .before = 1) %>%
      as.data.frame()
    ...
    plateFdTsM <- plateFdTs[-1] %>%
      map_df(~ fun_TGcorr(.x, plateFdTs, input$smtail)) %>%
      add_column(plateFdTs[[1]], .before = 1) %>%
      as.data.frame()
  ...
  a2MData <- reactive({
    ...
    plateMdT <- plateMC[-1] %>%
      map_df(~ fun_diff(.x, plateMC)) %>%
      map_df(~ fun_FtoT(.x, input$CalibT, input$calSlope)) %>%
      add_column(plateMC[, 1][-1], .before = 1) %>%
      as.data.frame()
    # Use the drivative->thrombin curve to make the alpha2M curve
    plateMdTM <- plateMdT[-1] %>%
      map_df(~ fun_TaM(.x, plateMdT, input$smtail)) %>%
      add_column(plateMdT[[1]], .before = 1) %>%
      as.data.frame()
  })
